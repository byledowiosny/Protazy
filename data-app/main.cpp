...
int main()
{
  int numberOfNames = filesList("ListOfSeriesNames.txt", "/home/jerzy/calculus/data/TROPO/");
  ...
  numberOfNames = filesList("ListOfSeriesNames.txt", "/home/jerzy/calculus/data/FINAL/");
  sortStringsFileAlphabet("ListOfSeriesNames.txt", numberOfNames);
...
void createFinalDataFile(const char *datFname, const char *resFname)
{
  ...
  string dirFinal = "/home/jerzy/calculus/data/FINAL/";
      ...
      for (int i=step; i<gapStart; i++)
        {
          ...
          final << QString::fromStdString(temp_dat).section(' ',0,1,QString::SectionSkipEmpty).
            toStdString() << endl;
        }
      QDateTime currentDate (QDateTime::fromString(QString::fromStdString(temp_dat).
                                                   section(' ',0,0,QString::SectionSkipEmpty),
                                                   datTimeFormat));
      for (int i=gapStart; i<=gapFinish; i++)
        {
          currentDate = currentDate.addSecs(3600);
          final << currentDate.toString(datTimeFormat).
            toStdString().c_str() << "      " << "0" << endl;  //'0' means empty
        }
      step = gapFinish+1;
    }
  while(temp[0] == 'G');
